...
// d3-free — colors.ts, which owns the actual `d3.scheme*` lookups, imports
 ...
 * PALETTES map (same key set either way) so this stays callable with no
 * `d3` global loaded — persist.ts depends on that.
 */
