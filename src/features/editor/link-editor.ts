 ...
 * (src/app/start-app.ts) owns `previousSortable`/the returned replacement rather than
 ...
 * listeners down.
 *
 * `state` is the stable, mutated-in-place reference from app/start-app.ts, so the
